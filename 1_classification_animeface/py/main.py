...
def load_data(args):
    ...
    train_dataset = AnimeFaceDataset(
        os.path.join(cwd, args.path2db, "train"), transform=train_transform
    )
    ...
    val_dataset = AnimeFaceDataset(
        os.path.join(cwd, args.path2db, "val"), transform=val_transform
    )
...
def main(args):
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=args.optimizer.lr_step_size, gamma=args.optimizer.lr_gamma
    )  # 学習率の軽減スケジュール
