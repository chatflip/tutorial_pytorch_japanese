...
if __name__ == '__main__':
    ...
    weight_name = "weight/AnimeFace_mobilenetv2_100.pth"
    ...
    torch.save(quantized_model.cpu().state_dict(), saved_weight)
    ...
    print('not model elapsed time = {0:d}h {1:d}m {2:d}s'.format(
        int(interval / 3600),
        int((interval % 3600) / 60),
        int((interval % 3600) % 60)))

    starttime = time.time()  # 実行時間計測(実時間)
    validate(args, quantized_model.to(device), device, val_loader, criterion, writer, iteration)
    # 実行時間表示
    endtime = time.time()
    interval = endtime - starttime
    print('quantized_model elapsed time = {0:d}h {1:d}m {2:d}s'.format(
        int(interval / 3600),
        int((interval % 3600) / 60),
        int((interval % 3600) % 60)))
